...
async def language_cmd(c: Client, message: types.Message) -> None:
    ...
    # Add back button
    keyboard.append([{
        "text": language_manager.get_text("back_button", current_lang),
    ...
    title = language_manager.get_text("language_title", current_lang)
    current_text = language_manager.get_text("language_current", current_lang, language=current_lang_name)
    select_text = language_manager.get_text("language_select", current_lang)
...
async def language_callback(c: Client, message: types.UpdateNewCallbackQuery) -> None:
    ...
    if data == "lang_back":
        ...
        await message.answer(language_manager.get_text("back_button", await language_manager.get_language(user_id, chat_id)))
        await message.edit_message_text(
            language_manager.get_text("start_home", await language_manager.get_language(user_id, chat_id), 
                                   user_name="User", bot_name=c.me.first_name, version="1.0", current_lang="English"),
            reply_markup=None  # Add start menu keyboard here
        )
        return
    ...
    if not language_manager.is_supported_language(lang_code):
        await message.answer(language_manager.get_text("error_invalid_request", await language_manager.get_language(user_id, chat_id)))
        return
    ...
    try:
        ...
        if success:
            ...
            success_msg = language_manager.get_text("language_changed", lang_code, language=lang_name)
            ...
            keyboard.append([{
                "text": language_manager.get_text("back_button", lang_code),
                "callback_data": "lang_back"
            }])
            
            reply_markup = types.ReplyMarkupInlineKeyboard(keyboard)
            
            title = language_manager.get_text("language_title", lang_code)
            current_text = language_manager.get_text("language_current", lang_code, language=lang_name)
            select_text = language_manager.get_text("language_select", lang_code)
            ...
            error_msg = language_manager.get_text("language_error", await language_manager.get_language(user_id, chat_id))
