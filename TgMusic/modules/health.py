...
async def show_stats(c: Client, msg: types.Message) -> None:
    ...
    try:
        ...
        report = format_stats_report(stats, user_lang)
        ...
        await reply_auto_delete_message(
            c, msg,
            "❌ Error generating statistics report",
            delay=10
        )


def format_stats_report(stats: Dict[str, Any], user_lang: str) -> str:
    """Format statistics into a readable report."""
    lines = ["📊 <b>Bot Statistics Report</b>", ""]
    
    # Bot metrics
    bot_stats = stats.get("bot", {})
    if bot_stats:
        lines.append("🤖 <b>Bot Metrics:</b>")
        lines.append(f"⏱ <b>Uptime:</b> {bot_stats.get('uptime_formatted', 'Unknown')}")
        lines.append(f"📊 <b>Total Commands:</b> {bot_stats.get('total_commands', 0)}")
        lines.append(f"❌ <b>Total Errors:</b> {bot_stats.get('total_errors', 0)}")
        lines.append(f"💬 <b>Active Chats:</b> {bot_stats.get('active_chats', 0)}")
        lines.append(f"🎵 <b>Total Playbacks:</b> {bot_stats.get('total_playbacks', 0)}")
        lines.append(f"❌ <b>Failed Playbacks:</b> {bot_stats.get('failed_playbacks', 0)}")
        lines.append("")
        
        # Cache statistics
        cache_stats = bot_stats.get("cache_stats", {})
        if cache_stats:
            lines.append("💾 <b>Cache Statistics:</b>")
            lines.append(f"🎯 <b>Cache Hits:</b> {cache_stats.get('hits', 0)}")
            lines.append(f"❌ <b>Cache Misses:</b> {cache_stats.get('misses', 0)}")
            lines.append(f"📈 <b>Hit Rate:</b> {cache_stats.get('hit_rate', 0):.1f}%")
            lines.append("")
        
        # Download statistics
        download_stats = bot_stats.get("download_stats", {})
        if download_stats:
            lines.append("⬇️ <b>Download Statistics:</b>")
            for platform, count in download_stats.items():
    ...
    if system_stats:
        lines.append("💻 <b>System Resources:</b>")
        lines.append(f"🖥️ <b>CPU Usage:</b> {system_stats.get('cpu_percent', 0):.1f}%")
        lines.append(f"💾 <b>Memory Usage:</b> {system_stats.get('memory_percent', 0):.1f}%")
        lines.append(f"💿 <b>Disk Usage:</b> {system_stats.get('disk_percent', 0):.1f}%")
        
        memory_available = system_stats.get('memory_available', 0)
        if memory_available > 0:
            memory_gb = memory_available / (1024**3)
            lines.append(f"💾 <b>Available Memory:</b> {memory_gb:.1f} GB")
        
        disk_free = system_stats.get('disk_free', 0)
        if disk_free > 0:
            disk_gb = disk_free / (1024**3)
            lines.append(f"💿 <b>Free Disk:</b> {disk_gb:.1f} GB")
        lines.append("")
    
    # Command breakdown
    command_breakdown = bot_stats.get("command_breakdown", {})
    if command_breakdown:
        ...
        for command, count in sorted_commands[:10]:  # Top 10 commands
            lines.append(f"  • {command}: {count}")
        lines.append("")
    
    # Footer
    lines.append("📅 <b>Report Generated:</b> " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
